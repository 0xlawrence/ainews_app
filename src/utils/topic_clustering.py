...
class TopicClusterer:
    ...
    async def _build_clusters_from_labels(
        ...
        for article, embedding, label in zip(articles, embeddings, cluster_labels, strict=False):
...
トピック名のみ回答してください："""
    ...
    async def identify_multi_source_topics(self, articles: list[ProcessedArticle]) -> dict[str, dict]:
        ...
        valid_articles = [art for art, emb in zip(articles, embeddings, strict=False) if emb is not None]
    ...
    def _validate_cluster_coherence(
        ...
        if not self._validate_topic_domain_coherence(filtered_articles):
            logger.info("Rejected filtered cluster due to remaining domain incoherence")
            return False, [], np.array([])
